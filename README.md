...
NOTA: Las expresiones regulares que debemos introducir en ```prometheus.yml``` deben estar escritas en Go. Para poder probarlas, existen herramientas online como [regex101](https://regex101.com/). Las expresiones regulares poseen una jerarquía; así, si por ejemplo tenemos la cadena ```hola mundo``` y definimos una expresión regular como ```'mund'```, esa expresión regular cogerá únicamente ```mund``` de la cadena entera. Como no se especifican paréntesis, el match se realiza en el nivel 0 (Group 0). En cambio, podemos hacer que ese mismo match se produzca en diferentes niveles: por ejemplo, que la parte ``mu`` se matchee a un nivel 0 y que la parte ```nd``` se matchee a un nivel 1 mediante la expresión regular ```mu(nd)```; todo lo que haya dentro del paréntesis se matcheará a nivel 1. Si utilizamos paréntesis dentro de paréntesis, a nivel 2 y así sucesivamente.
Por tanto, es importante aclarar que la expresión regular introducida en ```prometheus.yml``` deberá ser tal que aquella parte de la cadena de caracteres que queremos extraer debe matchear con la expresión regular en el Group 1, no en el 0. En otras palabras, si por ejemplo se desea obtener el _Request path_ de la string ```localhost:8080/users?x=15&y=45``` (esto es, la parte ```/users```), la expresión regular deberá ser ```[^/]+:[0-9]+(/.*)[?].*``` (probar en [regex101](https://regex101.com/)), ya que en ella estamos definiendo como primer nivel (Group 0) toda la cadena propiamente dicha, pero dentro del paréntesis encontramos la parte de la expresión regular con la que nos queremos quedar: ```(/.*)```, que en este caso equivale a ```/users```.
